...
def ttt(g, t):
    return np.arcsinh(g*t)/g


def xxx(g, t):
    return (np.sqrt(1+g**2*t**2)-1)/g
...
def calcul_dis_tau(T=3600*24*1000, g=9.81, pasdelta=3600*24*100):
    ...
    tau = np.append(tau, 2*ttt(g, T)+ttt(g, ts))
    dis = np.append(dis, 2*xxx(g, T)+xxx(-g, ts))
    vit = np.append(vit, -vvv(g, ts))
    t = np.append(t, 2*T+ts)

    # 3*T<t<4*T
    ts = np.arange(0, T+pas, pas)
    tau = np.append(tau, 4*ttt(g, T)-ttt(g, T-ts))
    dis = np.append(dis, xxx(g, T-ts))
    vit = np.append(vit, -vvv(g, T-ts))
    ...
    dis2 = V*t*(t < 2*T)+(2*V*T-V*(t-2*T))*(t >= 2*T)
